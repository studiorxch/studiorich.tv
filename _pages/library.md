...
{% assign genre_array = track.genre | default: empty %}

<a
class="track-card{% unless filtered_mood %} no-filter{% endunless %}"
href="/tracks/{{ track.slug }}/?autoplay=1"
data-mood="{{ mood_array | join: ' ' | downcase | escape }}"
data-genre="{{ genre_array | join: ' ' | downcase | escape }}"
